        ...
        <div class="card">
              ...
              <form action="" method="post">
                <div class="form-group">
                    ...
                    <input type="text" name="kode_suplier" class="form-control" id="kode_suplier" maxlength="10" placeholder="Masukan Kode Suplier" required>
                </div>
                <div class="form-group">
                    <label for="nama_suplier">Nama Suplier</label>
                    <input type="text" name="nama_suplier" class="form-control" id="nama_suplier" maxlength="15" placeholder="Masukan Nama Suplier" required>
                </div>
                <div class="form-group">
                    <label for="nama_pic">Nama PIC</label>
                    <input type="text" name="nama_pic" class="form-control" id="nama_pic" maxlength="15" placeholder="Masukan Nama PIC" required>
                </div>
                <div class="form-group">
                    <label for="kontak_pic">Kontak PIC</label>
                    <input type="text" name="kontak_pic" class="form-control" id="kontak_pic" maxlength="15" placeholder="Masukan Kontak PIC" required>
                </div>
                <div class="form-group">
                    <label for="alamat">Alamat</label>
                    <textarea type="text" name="alamat" class="form-control" id="alamat" placeholder="Masukan Alamat" required></textarea>
                </div>
                <div class="form-group">
                    <label for="website">Websie</label>
                    <input type="text" name="website" class="form-control" id="website"  placeholder="Masukan Website (skip jika tidak ada)" >
                </div>
                <div class="form-group">
                    <label for="akun_ig">Akun Instagram (Contoh : username_ig)</label>
                    <input type="text" name="akun_ig" class="form-control" id="akun_ig"  placeholder="Masukan Username Akun IG (skip jika tidak ada)" >
                </div>
                <div class="form-group">
                    <label for="akun_tiktok">Akun Tiktok (Contoh : @username_tiktok)</label>
                    <input type="text" name="akun_tiktok" class="form-control" id="akun_tiktok" placeholder="Masukan Username Akun Tiktok (skip jika tidak ada)" >
                </div>
...
if (isset($_POST['btn_tambah_mahasiswa'])) {
    ...
    $cek_kode_siplier = mysqli_query($conn, "SELECT kode_suplier FROM suplier WHERE kode_suplier = '$kode_suplier' ") 
    or die (mysqli_error($conn));
    $rv = mysqli_num_rows($cek_kode_siplier);

        if ($rv > 0 ) {
            echo '<script> alert("Kode Suplier Sudah Terdaftar! Input yang lain");
            window.location.href="../superadmin_suplier" </script>';

        } else {
            $query_simpan = mysqli_query($conn, "INSERT INTO suplier 
            VALUES 
            ('$kode_suplier', 
            '$nama_suplier',
            '$nama_pic',
